...
interface UnsplashGalleryPickerProps {
  title: string
  collectionId: string
  backgroundType: 'gradient' | 'pattern'
  queryKey: string
  maxPages: number
...
export default function UnsplashGalleryPicker({
  title,
  collectionId,
  backgroundType,
  queryKey,
  ...
  const fetchUnsplashPhotos = async (page: number) => {
    const response = await fetch(
      `https://api.unsplash.com/collections/${collectionId}/photos?page=${page}&per_page=30&q=100&fit=clip&w=1500&client_id=${process.env.NEXT_PUBLIC_UNSPLASH_ACCESS_KEY}`
    )
  ...
  const {
    ...
    queryFn: () => fetchUnsplashPhotos(currentPage),
  })
    ...
    <>
      ...
      <div className="mt-6 flex justify-end gap-2">
        <Button
          ...
          className="flex-center h-9 px-2.5 text-sm"
          onClick={() => {
