...
const CustomizedRnd = ({ children, className }: RndProps) => {
  ...
  const borderStyle: CSSProperties = {
    ...
    border: state.isHovering ? '3px solid #898aeb' : '3px solid transparent',
    ...
    >
      ...
      {state.isHovering && (
        <>
          ...
          <div className="absolute -right-2 top-1/2 h-4 w-4 border-[2px] border-border rounded-md bg-[#898aeb]"></div>
          <div className="absolute -left-2 top-1/2 h-4 w-4 border-[2px] border-border rounded-md bg-[#898aeb]"></div>
        </>
      )}
    </Rnd>
  )
